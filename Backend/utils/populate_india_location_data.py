...
def populate_india_location_data():
    ...
    with transaction.atomic():
        ...
        # Step 0: Ensure city categories exist before population
        if CityCategoriesMaster.objects.count() == 0:
            log("⚙️ CityCategoriesMaster is empty — creating default categories (A, B)")
            CityCategoriesMaster.objects.get_or_create(name="A", defaults={"description": "Major metro or category A cities"})
            CityCategoriesMaster.objects.get_or_create(name="B", defaults={"description": "Other cities"})
            log("✅ CityCategoriesMaster populated")
        ...
        default_category = CityCategoriesMaster.objects.filter(name='B').first() or category_a
