...
def validate_travel_entitlement(user, travel_mode, sub_option, city_category):
    ...
    Check if user's grade allows the requested travel mode/sub-option
    """
    from apps.master_data.models import GradeEntitlementMaster, TravelSubOptionMaster, CityCategoriesMaster
    from django.core.exceptions import ValidationError

    if not user.grade:
        raise ValidationError("User grade not defined. Cannot validate entitlements.")

    # Convert string → FK object if needed
    if isinstance(sub_option, str):
        sub_option = TravelSubOptionMaster.objects.filter(name__iexact=sub_option).first()
    if isinstance(city_category, str):
        city_category = CityCategoriesMaster.objects.filter(name__iexact=city_category).first()

    if not sub_option or not city_category:
        raise ValidationError(f"Invalid sub-option or city category reference.")
...
def validate_duplicate_travel_request(user, departure_date, return_date):
    ...
    if overlapping_requests.exists():
        ...
        raise ValidationError(
            ...
            f"({existing_request.status}). Please cancel or modify the existing request first."
        )
